...
export default function Listings() {
  ...
  const [enquiryModal, setEnquiryModal] = useState(null); // listing object
  const [postForm, setPostForm]     = useState({ farmer_name:"", farmer_phone:"", commodity:"Garlic", quantity_kg:"", price_inr_kg:"", state:"Madhya Pradesh", district:"", description:"" });
  ...
  const fetchListings = async () => {
    ...
    try {
      ...
      // fallback to mock data if Supabase not yet connected
  ...
  const handlePost = async (e) => {
    ...
    try {
      await createListing({ ...postForm, quantity_kg: Number(postForm.quantity_kg), price_inr_kg: Number(postForm.price_inr_kg) });
      setPostStatus("success");
      setPostForm({ farmer_name:"", farmer_phone:"", commodity:"Garlic", quantity_kg:"", price_inr_kg:"", state:"Madhya Pradesh", district:"", description:"" });
      fetchListings();
  ...
  return (
    <div>
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <button onClick={() => setActiveView(activeView === "browse" ? "post" : "browse")}
      ...
      {activeView === "browse" ? (
            ...
            {["All", ...CROPS].map(c => (
              <button key={c} onClick={() => setFilter(c)}
                ...
                style={{
                  ...
                  border: "1.5px solid",
                  borderColor: filter === c ? "#2d7a3a" : "#e5e7eb",
                }}>
        ...
        <div className="max-w-lg mx-auto bg-white rounded-2xl shadow-sm border border-green-100 overflow-hidden">
          <div className="px-6 py-4" style={{ background: "linear-gradient(135deg,#1e5c2a,#2d7a3a)" }}>
            ...
            <p className="text-green-200 text-sm">Reach UK buyers directly · 2% platform fee only</p>
          </div>
          <form onSubmit={handlePost} className="px-6 py-5 space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <FormField label="Your Name *" name="farmer_name" value={postForm.farmer_name} onChange={e => setPostForm(f => ({...f, farmer_name: e.target.value}))} required placeholder="Ramesh Patel" />
              <FormField label="Phone" name="farmer_phone" value={postForm.farmer_phone} onChange={e => setPostForm(f => ({...f, farmer_phone: e.target.value}))} placeholder="+91 98765 43210" />
            </div>
            <div className="grid grid-cols-2 gap-4">
              <div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <FormField label="Quantity (kg) *" name="quantity_kg" type="number" value={postForm.quantity_kg} onChange={e => setPostForm(f => ({...f, quantity_kg: e.target.value}))} required placeholder="5000" />
              <FormField label="Price (₹/kg)" name="price_inr_kg" type="number" value={postForm.price_inr_kg} onChange={e => setPostForm(f => ({...f, price_inr_kg: e.target.value}))} placeholder="55" />
            </div>
            <FormField label="District / Mandi" name="district" value={postForm.district} onChange={e => setPostForm(f => ({...f, district: e.target.value}))} placeholder="Mandsor" />
            <div>
            ...
            {postStatus === "error" && <p className="text-red-500 text-xs text-center">Failed to post. Please try again.</p>}
          </form>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4"
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden">
            <div className="px-6 py-4" style={{ background: "linear-gradient(135deg,#1e5c2a,#2d7a3a)" }}>
              ...
              <p className="text-green-200 text-sm">{enquiryModal.commodity} · {enquiryModal.quantity_kg}kg · {enquiryModal.farmer_name}</p>
            </div>
      ...
      <div className="px-5 py-4">
        <div className="flex items-start justify-between mb-3">
          ...
          {listing.is_ceta_eligible && (
            <span className="text-xs px-2 py-0.5 rounded-full font-semibold" style={{ background: "#e8f5ea", color: "#1e5c2a" }}>
              CETA ✓
            </span>
          )}
        </div>
        ...
        <div className="grid grid-cols-2 gap-2 mb-3">
          ...
          <div className="bg-orange-50 rounded-xl p-3 text-center">
            <div className="font-bold text-orange-700 text-lg">
              {listing.price_inr_kg ? `₹${listing.price_inr_kg}/kg` : "Negotiable"}
            </div>
        ...
        {listing.description && (
          <p className="text-xs text-gray-500 mb-3 leading-relaxed line-clamp-2">{listing.description}</p>
        )}

        <div className="flex items-center justify-between">
          <div className="text-xs text-gray-400">by {listing.farmer_name}</div>
...
function FormField({ label, ...props }) {
  return (
    <div>
      ...
      <input {...props}
        className="w-full rounded-xl border border-gray-200 px-3 py-2.5 text-sm focus:outline-none focus:border-green-500 bg-gray-50 transition-colors"/>
    </div>
  );
}

// ── Fallback mock data if Supabase not yet connected ──────────────────────────
const MOCK_LISTINGS = [
  { id:"1", farmer_name:"Ramesh Patel",   commodity:"Garlic",   quantity_kg:5000,  price_inr_kg:55,  state:"Madhya Pradesh", district:"Mandsor",    description:"Fresh white garlic, export grade, post-harvest treated", is_ceta_eligible:true },
  ...
  { id:"3", farmer_name:"Kavita Sharma",  commodity:"Turmeric", quantity_kg:2000,  price_inr_kg:98,  state:"Telangana",      district:"Nizamabad",  description:"7% curcumin, machine cleaned, export quality", is_ceta_eligible:true },
  { id:"4", farmer_name:"Murugan Pillai", commodity:"Ginger",   quantity_kg:3000,  price_inr_kg:42,  state:"Kerala",         district:"Wayanad",    description:"Baby ginger, fresh, unwashed, export quality", is_ceta_eligible:true },
  { id:"5", farmer_name:"Anjali Reddy",   commodity:"Chilli",   quantity_kg:1500,  price_inr_kg:76,  state:"Andhra Pradesh", district:"Guntur",     description:"Guntur Sannam S4, dried, 12-15% moisture", is_ceta_eligible:true },
  { id:"6", farmer_name:"Vikram Singh",   commodity:"Potato",   quantity_kg:10000, price_inr_kg:14,  state:"Uttar Pradesh",  district:"Agra",       description:"Chipsona variety, washed, 40-60mm size", is_ceta_eligible:true },
];
